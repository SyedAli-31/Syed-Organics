...
const HeaderMenu = ({ categories }: { categories: CATEGORIES_QUERYResult }) => {
  ...
  return (
    <div className="hidden md:inline-flex w-1/3 items-center gap-3 text-sm capitalize font-semibold">
      <Link
        href={"/"}
        className={`hover:text-green-700 hoverEffect relative group ${
          pathname === "/" && "text-darkColor"
        }`}
      >
        Home
        <span
          className={`absolute -bottom-0.5 left-1/2 w-0 h-0.5 bg-green-700 hoverEffect group-hover:w-1/2 group-hover:left-0 ${
            pathname === "/" && "w-1/2"
          }`}
        />
        <span
          className={`absolute -bottom-0.5 right-1/2 w-0 h-0.5 bg-green-700 hoverEffect group-hover:w-1/2 group-hover:right-0 ${
            pathname === "/" && "w-1/2"
          }`}
        />
      </Link>
